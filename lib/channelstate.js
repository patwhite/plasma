...
function ChannelState(options) {
  ...
  this.theirCurrentRevocation = encoding.ZERO_KEY; // public
  this.theirCurrentRevHash = encoding.ZERO_HASH;
...
ChannelState.prototype.fromOptions = function(options) {
  ...
  if (options.ourCommitKey) {
    ...
    this.ourCommitPub = secp256k1.publicKeyCreate(this.ourCommitKey, true);
  }
  ...
  if (options.ourMultisigKey) {
    ...
    this.ourMultisigPub = secp256k1.publicKeyCreate(this.ourMultisigKey, true);
  }
